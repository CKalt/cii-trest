...
//import com.cii.bean.StandardHashCodeEquals;
//import com.cii.dto.CourtDTO;
import com.fasterxml.jackson.annotation.JsonInclude;

//@Document
@JsonInclude(JsonInclude.Include.NON_NULL)
//public class Court extends StandardHashCodeEquals {
...
public class Court {
	...
	@Transient
	private List<String> allCaseTypes;

	@Transient
	private List<String> parentCaseTypes;
	...
	public Court() {
		setOption("recordsDOBsInUTC", Boolean.FALSE);
		setOption("queueTime", new Integer(4));
		setOption("usesLogin", Boolean.FALSE);
		setOption("uses2FA", Boolean.FALSE);
	}
	...
	public void addDefaultMessage(String id, String val) {
		defaultMessages.put(id, val);
	}
	...
	public void addMessage(String id, String val) {
		messages.put(id, val);
	}
	...
	public CaseTypeConfig getCaseTypeConfig(String caseType) {
		return caseTypeConfigs.get(caseType);
	}

	public Map<String, CaseTypeConfig> getAllCaseTypeConfigs() {
	...
	public List<String> getAllCaseTypes() {
		if (allCaseTypes == null) {
			...
			for (String parent : caseTypeConfigs.keySet()) {
				caseTypeSet.add(parent);
				caseTypeSet.addAll(caseTypeConfigs.get(parent).getSubTypes());
			}
			allCaseTypes = new ArrayList<String>(caseTypeSet);
		}
		return allCaseTypes;
	}

	public void addParentCaseType(String newCaseType, Date startDate, Date endDate, String caption) {
		CaseTypeConfig newConfig = new CaseTypeConfig();
		List<String> subTypes = new ArrayList<>();
		subTypes.add(newCaseType);
		newConfig.setSubTypes(subTypes);
		if (endDate != null) {
		...
		if (caption != null) {
			newConfig.setCaption(caption);
		}
		caseTypeConfigs.put(newCaseType, newConfig);
	}

	public void addSubType(String parent, String subType) {
		if (parent == null || subType == null) {
			return;
		}
		caseTypeConfigs.get(parent).addSubType(subType);
	}

	public String getParentCaseType(String subType) {
		for (String parent : caseTypeConfigs.keySet()) {
			if (caseTypeConfigs.get(parent).getSubTypes().contains(subType)) {
				return parent;
			}
		}
		return null;
	...
	public List<String> getSubTypes(String parent) {
		return caseTypeConfigs.get(parent).getSubTypes();
	}

	public List<String> getParentCaseTypes() {
		if (parentCaseTypes == null) {
			parentCaseTypes = new ArrayList<String>(caseTypeConfigs.keySet());
		}
		return parentCaseTypes;
	}

	public List<String> getActiveCaseTypes() {
		List<String> activeCaseTypes = new ArrayList<String>();
		for (String ct : caseTypeConfigs.keySet()) {
			if (caseTypeConfigs.get(ct).isValid()) {
				activeCaseTypes.add(ct);
			}
		}
		return activeCaseTypes;
	}

	public String getConfiguration(String caseType) {
		return configurations.get(caseType);
	}
	...
	public void addConfiguration(String caseType, String configurationId) {
		configurations.put(caseType, configurationId);
	}
	...
	public void setUrlKey(String urlKey) {
		this.urlKey = (urlKey == null) ? null : urlKey.trim().toUpperCase();
	}
	...
	public int getIntOption(String key) {
		Integer value = (Integer) getOption(key);
		return (value != null) ? value.intValue() : 0;
	}

	public boolean getBooleanOption(String key) {
		Boolean value = (Boolean) getOption(key);
